...
SELECT mnr,
       ...
       gbdatum
FROM medewerkers
WHERE date_part('year', gbdatum) < '1980'
    AND (functie = 'VERKOPER'
         OR functie = 'TRAINER'); -- S4.2.
...
CREATE OR REPLACE VIEW s4_5 AS -- [TEST]
...
SELECT (EXTRACT(YEAR FROM MAX(AGE(gbdatum))) - EXTRACT(YEAR FROM
MIN(AGE(gbdatum))))
AS verschil, TRUNC((AVG(EXTRACT(YEAR FROM AGE(gbdatum)))), 0) AS gemiddeld
FROM medewerkers;
...
SELECT COUNT(*) as aantal_medewerkers,
       round(SUM(comm)/COUNT(*), -1) AS commissie_medewerkers,
    (SELECT TRUNC(AVG(comm), 0) AS commissie_verkopers
     FROM medewerkers
